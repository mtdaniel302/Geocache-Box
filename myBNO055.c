...
//comres = BNO055_I2C_bus_read(BNO055_I2C_ADDR1, BNO055_CHIP_ID_ADDR, read_data, 1); //read 1 byte into read_data
...
/************************* START READ RAW FUSION DATA ********
	//comres stands for communication result
	...
	*FUSION MODE
		...
		*0x0B - OPERATION_MODE_NDOF_FMC_OFF //FMC = Fast Magnetometer Calibration
...
s8 BNO055_I2C_bus_write(u8 dev_addr, u8 reg_addr, u8 *reg_data, u8 cnt)
{
	...
	u8 stringpos = BNO055_ZERO_U8X; //0
	//array[BNO055_ZERO_U8X] = reg_addr;

	/* start of my added i2c read function */
	...
	/* end of my added i2c write function */
...
s8 BNO055_I2C_bus_read(u8 dev_addr, u8 reg_addr, u8 *reg_data, u8 cnt)
{
	...
	/*I can't figure the above out, but I put in my I2C read function here*/

	//i2c_start_wait(dev_addr+I2C_WRITE);     // set device address and write mode
	//i2c_write(reg_addr);                    // write address
	//i2c_rep_start(dev_addr+I2C_READ);       // set device address and read mode
	//*reg_data = i2c_readNak();                 // read one byte


	for (stringpos = BNO055_ZERO_U8X; stringpos < cnt; stringpos++)
	{
		i2c_start_wait(dev_addr+I2C_WRITE);     // set device address and write mode
		i2c_write(reg_addr + stringpos);        // write address
		i2c_rep_start(dev_addr+I2C_READ);       // set device address and read mode
		*(reg_data + stringpos) = i2c_readNak();
	}
...
/*---------------------------------------------------------------------------*
 *********************** START GET CALIBRATION FUNCTION**************
 *--------------------------------------------------------------------------*/

void getCalibration(uint8_t *sys, uint8_t *gyro, uint8_t *accel, uint8_t *mag)
{
	uint8_t *calData = 0;
	BNO055_I2C_bus_read(MY_BNO055_I2C_ADDDRESS, BNO055_CALIB_STAT_ADDR, calData, 1);
...
/*---------------------------------------------------------------------------*
 *********************** END GET CALIBRATION HELPER FUNCTION**************
 *--------------------------------------------------------------------------*/